...
export function PenLayer(props: {
  ...
  const paths = useMemo(() => {
    return props.strokes.map((s) => ({ id: s.id, d: pointsToPath(s.points), width: s.width }))
  }, [props.strokes])
  ...
  const onPointerMove = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    props.setStrokes(
      props.strokes.map((s) => (s.id === id ? { ...s, points: [...s.points, p] } : s)),
    )
  }
  ...
  return (
    <svg
      className={pointerEnabled ? 'tc-penLayer tc-penLayerActive' : 'tc-penLayer'}
      width={size.width}
      height={size.height}
