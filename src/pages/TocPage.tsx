      ...
      topBar={
        <div className="flex items-center justify-between w-full h-full">
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-xl font-bold text-slate-800 font-serif border-b-2 border-slate-300 pb-1">
              İçindekiler
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto py-16 px-6">
        {lastTopic ? (
          <div className="mb-16 p-8 bg-[#fffdfa] rounded-sm border-2 border-slate-200 shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 relative before:absolute before:inset-0 before:border-[6px] before:border-double before:border-slate-100/50 before:pointer-events-none before:rounded-sm">
            <div className="relative z-10">
              <div className="text-sm font-bold text-indigo-700 uppercase tracking-widest mb-2 font-sans">
                Son kaldığın yer
              </div>
            ...
            <button
              type="button"
              className="relative z-10 px-8 py-3 bg-slate-900 text-white font-bold rounded-sm hover:bg-slate-800 transition-colors shadow-sm font-serif"
              onClick={goLast}
            >
              Devam et
            </button>
        ...
        <div className="space-y-12">
          {units.map((u) => (
            <details
              key={u.id}
              className="group bg-[#fffdfa] rounded-sm border border-slate-300 shadow-sm overflow-hidden"
              open
            >
              <summary className="cursor-pointer p-8 bg-slate-100/50 font-bold text-2xl text-slate-900 flex items-center justify-between select-none font-serif border-b border-slate-200">
                {u.title}
                <svg
                  className="w-6 h-6 text-slate-500 group-open:rotate-180 transition-transform"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
